	...
	echo'<div>
			...
			<table class="form-table" border="1">
				<tr>
					...
					<td align="center"><b>'.__('Count', 'wm_cf7_userto_mchimp').'</b></td>
					<td align="center"><b>'.__('id', 'wm_cf7_userto_mchimp').'</b></td>
					<td align="center"><b>'.__('Subject', 'wm_cf7_userto_mchimp').'</b></td>
					<td align="center"><b>'.__('Perm. reminder', 'wm_cf7_userto_mchimp').'</b></td>
					<td align="center"><b>'.__('CF7 Field', 'wm_cf7_userto_mchimp').'</b></td>
				<tr>';
				if(isset($res->lists) && is_array($res->lists)){
					foreach($res->lists as $resi){
						echo'<tr>
								<td>'.esc_html($resi->name).'</td>
								<td>'.esc_html($resi->stats->member_count).'</td>
								<td>'.esc_html($resi->id).'</td>
								<td>'.esc_html($resi->campaign_defaults->subject).'</td>
								<td>'.esc_html($resi->permission_reminder).'</td>
								<td>[hidden wm_mchp_cf7_audience_id default:"'.esc_html($resi->id).'"]</td>
							<tr>';
					}
				}
			echo'</table>
...
if($tab==3){
	...
	echo'<div>
			...
			<form action="'.$_SERVER['REQUEST_URI'].'" method="POST">
				...
				echo'</select></td></tr>
					<tr><td><input type="text" name="wm_subscriber_fname" placeholder="'.__('First Name', 'wm_cf7_userto_mchimp').'"></td></tr>
					<tr><td><input type="text" name="wm_subscriber_lname" placeholder="'.__('Last Name', 'wm_cf7_userto_mchimp').'"></td></tr>
					<tr><td><input type="text" name="wm_subscriber_email" placeholder="'.__('E-Mail', 'wm_cf7_userto_mchimp').'"></td></tr>
					<tr><td>
							<p class="submit">
...
if($tab==5){
	...
	echo'<div>
			<h1>'.__('MailChimp Credentials to API', 'wm_cf7_userto_mchimp').'</h1>
			<form action="'.$_SERVER['REQUEST_URI'].'" method="POST">
				<table class="form-table">
					<tr><td><input type="text"        name="mailchimp_key"         value="'.esc_html(\WM_USRTOMC_ns\WM_USRTOMC::wm_get_key()).'" placeholder="'.__('MailChimp API KEY', 'wm_cf7_userto_mchimp').'"></td></tr>
					<tr><td>
							<p class="submit">
